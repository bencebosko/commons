package com.bencesoft.commons.validation.constants;

public class ValidationErrorCodes {

    public static final String EMAIL = "email-validation-error";
    public static final String NON_NEGATIVE = "non-negative-validation-error";
    public static final String NON_NULL = "non-null-validation-error";
    public static final String NOT_BLANK = "not-blank-validation-error";
    public static final String PERSON_NAME = "person-name-validation-error";
    public static final String STRONG_PASSWORD = "strong-password-validation-error";
    public static final String USERNAME = "username-validation-error";
}
